...
class DropPath(nn.Module):
    ...
    def forward(self, x):
        return drop_path(x, self.drop_prob, self.training)  # ？？？ 没有self.training
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=GELU, drop=0.0):
        ...
        self.act = act_layer()
        self.drop1 = nn.Dropout(drop_probs[1])
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4, qkv_bias=False, drop=0., attn_drop=0., drop_path=0.0, act_layer=GELU, norm_layer=nn.LayerNorm):
        super().__init__()
